...
void Seq05::timer_callback()
{
    std::lock_guard<std::mutex> lock(mutex_); // for clean memory
    ...
    // make a message be the current iteration number
    MsgRecord record;
    record.id = counter_;
    clock_gettime(CLOCK_MONOTONIC, &record.time); //record time of message sent
...
void Seq05::loop_callback(const std_msgs::msg::UInt64::SharedPtr msg)
{
    // make a record of message received
    MsgRecord record;
    record.id = msg->data;
    clock_gettime(CLOCK_MONOTONIC, &record.time); // record time of message received
    receives_.push_back(record);
}

void Seq05::post_process()
{
